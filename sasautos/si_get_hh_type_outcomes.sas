	...
	proc sql;
		...
		create table _temp_households_people_age as 
			...
			select a.&si_hht_id_col. 
				, datediff(yyyy, datefromparts(c.snz_birth_year_nbr , c.snz_birth_month_nbr, 15), a.&si_hht_asat_date.) as age_as_at_date
				, b.[snz_idi_address_register_uid]
				,
			    case 
				  when datediff(yyyy, datefromparts(c.snz_birth_year_nbr , c.snz_birth_month_nbr, 15), a.&si_hht_asat_date.) < 5 then 1 
			    end as age_under_5
		      	,
	         	case 
		        	when datediff(yyyy, datefromparts(c.snz_birth_year_nbr , c.snz_birth_month_nbr, 15), a.&si_hht_asat_date.) between 5 and 12 then 1 
		        end as age_school_5_12
	         	,
	            case 
		            when datediff(yyyy, datefromparts(c.snz_birth_year_nbr , c.snz_birth_month_nbr, 15), a.&si_hht_asat_date.) between 13 and 18 then 1 
              	end as age_teen_13_18
	            ,
                case 
	                when datediff(yyyy, datefromparts(c.snz_birth_year_nbr , c.snz_birth_month_nbr, 15), a.&si_hht_asat_date.) between 19 and 64 then 1 
                end as age_adult_19_64
                ,
                case 
                    when datediff(yyyy, datefromparts(c.snz_birth_year_nbr , c.snz_birth_month_nbr, 15), a.&si_hht_asat_date.) > 64 then 1 
                end as age_over_64
