...
INPUT:
si_diabetes_dsn = Database name {default = IDI_Clean}
si_diabetes_proj_schema = Project schema used to find your tables in the sandpit
si_diabetes_table_in = name of the input table
si_diabetes_id_col = id column used for joining tables {default = snz_uid}
si_diabetes_asat_date = name of the column containing a datetime field used to identify outcomes 
in a specified time period

OUTPUT:
si_diabetes_table_out = name of the output table containing a diabetes flags diabetes1
...
NOTES: 
Chronic conditions might be just enough to identify all diabetes - but used Rob code which combines 
public discharges as well!
...
%macro si_get_diabetes( si_diabetes_dsn = IDI_Clean, si_diabetes_proj_schema =, si_diabetes_table_in =, 
					...
					select moh_dia_event_id_nbr 
						from [&si_diabetes_dsn.].[moh_clean].[pub_fund_hosp_discharges_diag]
							...
							from [&si_diabetes_dsn.].[moh_clean].[nnpac] 
								where  moh_nnp_purchase_unit_code in ('M20006', 'M20007', 'M20004','M20005','M20010','M20015',
									'MAOR0106')
									...
									inner join (
										select chemical_id,DIM_FORM_PACK_SUBSIDY_KEY
											from [IDI_Metadata].[clean_read_CLASSIFICATIONS].[moh_dim_form_pack_subsidy_code]
												where chemical_id in (1567,1568,1569,2277,2800,1649,1655,6300,3857,1192,3783,
													1648,1110,1247,1572,2158,3802,3807,1794,1570,1068,2276,3739)) c
	...
	proc sort data=diabetes1 out=diabetes2(where=(diabetes1=1));
		by snz_uid;
	run;
	...
	data diabetes4 (keep=snz_uid);
		...
		if  moh_lab_test_code = 'BP8' then
			microalbumin+1;

		if  moh_lab_test_code = 'BP8' then
			lastmicroalbumin=eventsasdate;

		if (count_hba>=4) and ((hba_1-hba_4)<366) and (microalbumin and (hba_4<= lastmicroalbumin <=hba_1)) then
			diabetes_lab=1;

		if last.snz_uid and diabetes_lab then
			do;
				output;
			end;
	run;
	...
	proc sort data=diabetes5 out=diabetes6(rename=(snz_uid=&si_diabetes_id_col.)) nodupkey;
		by snz_uid;
	run;
